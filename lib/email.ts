...
function getFromEmail() {
  return process.env.RESEND_FROM_EMAIL || 'Inferox <onboarding@resend.dev>';
}
...
export async function sendContactEmails(data:{name:string;email:string;label?:string;subject?:string;message:string}) {
  ...
  await Promise.all([
    OWNER ? resend.emails.send({ from: FROM, to: OWNER, subject:`Contact: ${data.subject||data.name}`, html: ownerHtml, reply_to: data.email }) : Promise.resolve(),
